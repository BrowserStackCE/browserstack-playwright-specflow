...
namespace  SpecFlowPlaywrightXUnitExample.Steps
{
    ...
    internal class PlaywrightTestParallel
    {
        ...
        public async Task SearchWithGoogle()
        {
            ...
            await _page.Locator("[aria-label='Search']").ClickAsync();
            await _page.FillAsync("[aria-label='Search']", "BrowserStack");
            await _page.Locator("[aria-label='Google Search'] >> nth=0").ClickAsync();
        }
    }
}
